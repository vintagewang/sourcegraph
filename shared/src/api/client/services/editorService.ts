...
import { BehaviorSubject, combineLatest, EMPTY, from, Subscribable } from 'rxjs'
...
export interface EditorService {
     ...
     *
     * The model text are omitted for performance reasons and because there are not currently any
     * callers that need to observe all editors and their model text. Only the model's URI and
     * language are included.
     *
     * This observable emits frequently. Prefer using {@link EditorService#editors} when you only
     * need to observe editor properties (and not the model), or
     * {@link EditorService#observeEditorAndModel} when you only need to observe a single editor's
     * properties and model text.
     */
     ...
     * @returns An observable that emits when the editor or its model changes and never completes.
     * It does not emit if no such editor exists or the editor is removed.
     */
